...
void main() {
	...
	//albedo = 	texSample2D(g_Texture0, v_TexCoord[5]).rgb * 0.22f +
	//				texSample2D(g_Texture0, v_TexCoord[6]).rgb * 0.56f +
	//				texSample2D(g_Texture0, v_TexCoord[7]).rgb * 0.22f;
	
	//float desat = dot(albedo, vec3(0.299, 0.587, 0.114));
	//albedo = mix(vec3(desat, desat, desat), albedo, 0.5);
	//albedo *= 1.5;
